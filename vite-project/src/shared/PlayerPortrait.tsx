...
export function PlayerPortrait({ portrait, logo = '', label, slot }: { portrait: string; logo?: string; label: string; slot: number }) {
  ...
  return source ? <img className="player-portrait" src={source} alt={label} onError={() => setFailed(previous => [...previous, source])} /> : <span className="empty portrait-placeholder">{String(slot + 1).padStart(2, '0')}</span>;
}
