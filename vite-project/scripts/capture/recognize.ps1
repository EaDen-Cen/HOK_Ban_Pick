...
if ($null -ne $inputData.regions) {
  $previews = @()
  foreach ($region in $inputData.regions) { $previews += Capture-Region $region }
  @{ previews=$previews } | ConvertTo-Json -Compress -Depth 4
} else {
  @{ preview=(Capture-Region $inputData.region) } | ConvertTo-Json -Compress -Depth 4
}
